...
const OtherDepositSetting = () => {
  const [activeTab, setActiveTab] = useState(0);
  const [depositBonuses, setDepositBonuses] = useState({ SECOND: [], THIRD: [] });
  ...
  const fetchDepositBonuses = async () => {
    try {
      ...
      // Find the SECOND and THIRD deposit types from the response array
      const secondDeposit = response.data.find(item => item.depositType === "SECOND");
      const thirdDeposit = response.data.find(item => item.depositType === "THIRD");

      setDepositBonuses({
        SECOND: secondDeposit?.rules || [],
        THIRD: thirdDeposit?.rules || []
      });
    } catch (err) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axiosInstance.post(
        ...
        {
          depositType: activeTab === 0 ? "SECOND" : "THIRD",
  ...
  return (
    <ThemeProvider theme={darkTheme}>
      <Box sx={{ margin: "0 auto", p: 3, backgroundColor: '#0f172a !important', minHeight: '100vh', borderRadius: '16px' }}>
      <Typography
        variant="h5"
        sx={{
          mb: 4,
      ...
      >
        <Tab label="Second Deposit" />
        <Tab label="Third Deposit" />
      </Tabs>

      <Paper elevation={3} sx={{ p: 3, mb: 4, background: '#0f172a', border: '1px solid rgba(148, 163, 184, 0.12)' }}>
      ...
      <Paper elevation={3} sx={{ p: 3, background: '#0f172a', border: '1px solid rgba(148, 163, 184, 0.12)' }}>
        <Typography
          variant="h5"
          sx={{
            mb: 3,
        ...
        >
          Current {activeTab === 0 ? "Second" : "Third"} Deposit Bonus Rules
        </Typography>
        ...
        <TableContainer>
          <Table>
            ...
            <TableBody>
              {depositBonuses[activeTab === 0 ? "SECOND" : "THIRD"].map((bonus) => (
...
export default OtherDepositSetting;